...
const ProductsListingByArt = () => {
  ...
  useEffect(() => {
    ...
    const storedData = sessionStorage.getItem(product);
    ...
    if (storedData) {
      ...
      fetch(`http://127.0.0.1:8000/api/all_products_article/${product}`)
        .then((response) => {
          if (!response.ok) {
        ...
        .then((data) => {
          ...
          if (data?.safe && data?.products_data) {
            ...
            setTimeout(() => {
              ...
              sessionStorage.setItem(`${product}`, JSON.stringify(data.products_data));
            }, 0);
          } else {
  ...
  return (
    <div className="p-6 mx-auto w-[100vw] min-h-[100vh]">
      <h2 className="text-xl font-semibold mb-4">Products in {product}</h2>
...
export default ProductsListingByArt;
